 ...
 *     ...
 *   ]
 * @returns {number[]} - An array containing the 'power' of each game.
 *                       The maximum of each r, g b, multiplied together.
 */
const checkGamesPower = (lines = []) =>
  lines
    .map((line) => [...getDraws(line)])
    .filter((x) => x.length !== 0)
    .map((game) => [
      Math.max(...game.map((draw) => draw.at(0))),
      Math.max(...game.map((draw) => draw.at(1))),
      Math.max(...game.map((draw) => draw.at(2))),
    ])
    .map((draw) => draw.reduce((acc, cur) => acc * cur, 1));

/**
 * Sums the numbers of valid games.
 *
 * @param {number[]} games - An array containing the numbers of valid games.
 * @returns {number} - The sum of the power of each game.
 */
const sumGamesPower = (games = []) => games.reduce((acc, cur) => acc + cur, 0);
...
const main = () => {
  ...
  if (fileName) {
    const result = sumGamesPower(
      checkGamesPower(parseLines(fileContentToArray(fileName))),
    );
